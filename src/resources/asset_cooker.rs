...
impl AssetCooker {
    ...
    pub fn new<P1: AsRef<Path>, P2: AsRef<Path>>(input: P1, output: P2) -> ReactorResult<Self> {
        ...
        fs::create_dir_all(&output_dir).map_err(|e| {
            ReactorError::new(
                crate::core::error::ErrorCode::IoError,
    ...
    pub fn cook_file(&mut self, path: &Path) -> ReactorResult<()> {
        ...
        let metadata = fs::metadata(path).map_err(|e| {
            ReactorError::new(
                crate::core::error::ErrorCode::IoError,
                format!("Failed to get filesystem metadata: {}", e),
            )
        })?;
        ...
        match asset_type {
            AssetType::Texture => {
                ...
                let img = image::load_from_memory(&content).map_err(|e| {
                    ReactorError::asset_load(format!("Failed to parse image data: {}", e))
                })?;
                ...
                extra.insert(
                    "mipmaps".to_string(),
                    serde_json::Value::from(mip_levels.len()),
                );
            }
            AssetType::Model => {
                ...
                if ext == "glb" || ext == "gltf" {
                    ...
                    if modified {
                        fs::write(path, &file_bytes).map_err(|e| {
                            ReactorError::new(
                                crate::core::error::ErrorCode::IoError,
                                format!("Failed to write patched model: {}", e),
                            )
                        })?;
                ...
                let (vertices, indices) = crate::resources::asset_manager::load_model_auto(path)
                    .map_err(|e| {
                        ReactorError::asset_load(format!("Failed to parse model file: {}", e))
                    })?;
                ...
                extra.insert(
                    ...
                    serde_json::Value::from(vertices.len()),
                );
                extra.insert(
                    "index_count".to_string(),
                    serde_json::Value::from(indices.len()),
                );
            }
            AssetType::Audio => {
                // Parse audio header data using AudioClip
                let clip = crate::systems::audio::AudioClip::from_bytes(path_str.clone(), &content)
                    .map_err(|e| {
                        ReactorError::asset_load(format!(
                            "Failed to parse audio file metadata: {}",
                            e
                        ))
                    })?;
                ...
                extra.insert(
                    ...
                    serde_json::Value::from(clip.sample_rate),
                );
                extra.insert(
                    "channels".to_string(),
                    serde_json::Value::from(clip.channels),
                );
                extra.insert(
                    "duration".to_string(),
                    serde_json::Value::from(clip.duration),
                );
            }
