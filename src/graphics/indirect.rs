...
impl IndirectDrawBuffer {
    pub fn new(device: Arc<ash::Device>, allocator: &mut gpu_allocator::vulkan::Allocator, max_commands: u32) -> ReactorResult<Self> {
        ...
        let buffer_info = vk::BufferCreateInfo::default()
            .size(size)
            .usage(vk::BufferUsageFlags::INDIRECT_BUFFER | vk::BufferUsageFlags::STORAGE_BUFFER | vk::BufferUsageFlags::TRANSFER_DST)
    ...
    pub fn push(&mut self, cmd: IndirectCommandWithMaterial) -> ReactorResult<()> {
        ...
        if let Some(mapped) = self.allocation.mapped_ptr() {
            unsafe {
                ...
                std::ptr::copy_nonoverlapping(&cmd as *const _ as *const u8, ptr.add(offset), stride);
            }
        }
        self.count += 1;
        Ok(())
    ...
    pub fn record_draw(&self, cmd: vk::CommandBuffer) {
        ...
        unsafe { self.device.cmd_draw_indexed_indirect(cmd, self.buffer, 0, self.count, stride); }
    }
...
impl Drop for IndirectDrawBuffer {
    fn drop(&mut self) { unsafe { self.device.destroy_buffer(self.buffer, None); } }
}
