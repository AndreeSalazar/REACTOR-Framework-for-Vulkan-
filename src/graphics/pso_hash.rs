...
impl PsoHashBuilder {
    ...
    pub fn hash_vertex_input(&mut self, vi: &vk::PipelineVertexInputStateCreateInfo) -> &mut Self {
        ...
        for b in bindings { b.binding.hash(&mut self.hasher); b.stride.hash(&mut self.hasher); (b.input_rate as u32).hash(&mut self.hasher); }
        ...
        for a in attrs { a.location.hash(&mut self.hasher); a.binding.hash(&mut self.hasher); (a.format as u32).hash(&mut self.hasher); a.offset.hash(&mut self.hasher); }
        self
    }
    ...
    pub fn hash_rasterization(&mut self, r: &vk::PipelineRasterizationStateCreateInfo) -> &mut Self {
        ...
        (r.polygon_mode as u32).hash(&mut self.hasher);
        (r.cull_mode.as_raw() as u32).hash(&mut self.hasher);
        (r.front_face as u32).hash(&mut self.hasher);
    ...
    pub fn hash_multisample(&mut self, m: &vk::PipelineMultisampleStateCreateInfo) -> &mut Self {
        (m.rasterization_samples.as_raw() as u32).hash(&mut self.hasher);
    ...
    pub fn hash_depth_stencil(&mut self, d: &vk::PipelineDepthStencilStateCreateInfo) -> &mut Self {
        ...
        (d.depth_compare_op as u32).hash(&mut self.hasher);
    ...
    pub fn hash_render_pass_formats(&mut self, color: &[vk::Format], depth: Option<vk::Format>) -> &mut Self {
        ...
        for f in color { (*f as u32).hash(&mut self.hasher); }
        if let Some(df) = depth { (df as u32).hash(&mut self.hasher); } else { 0u32.hash(&mut self.hasher); }
        self
    }
